...
//----------------------------------------------------------------------------------------------------------
__global__ __launch_bounds__(TPBJH)
void quark_jh512_gpu_hash_64(uint32_t threads, uint32_t* g_hash){
	...
	if (thread < threads){
		...
		*(uint28*)&hash[0] = __ldg4((uint28*)&Hash[0]);
		*(uint28*)&hash[8] = __ldg4((uint28*)&Hash[8]);
		...
		*(uint28*)&Hash[0] = *(uint28*)&x[4][0];
		*(uint28*)&Hash[8] = *(uint28*)&x[6][0];
	}
}
//----------------------------------------------------------------------------------------------------------
...
__global__ __launch_bounds__(TPBK52,7)
#else
__global__ __launch_bounds__(TPBK50,3)
#endif
void quark_keccak512_gpu_hash_64(uint32_t threads, uint2* g_hash){
	...
	if (thread < threads){
		...
		uint28* d_hash = (uint28 *)&g_hash[thread * 8];

		#if __CUDA_ARCH__ > 500
		*(uint28*)&s[ 0] = __ldg4(&d_hash[ 0]);
		*(uint28*)&s[ 4] = __ldg4(&d_hash[ 1]);
		#else
		*(uint28*)&s[ 0] = d_hash[ 0];
		*(uint28*)&s[ 4] = d_hash[ 1];		
		#endif
...
__host__ void cuda_jh512Keccak512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t *d_hash){
	...
	if (device_sm[dev_id] <= 500) tpb = TPBK50;
	dim3 gridK((threads + tpb-1)/tpb);
	dim3 blockK(tpb);
	quark_keccak512_gpu_hash_64<<<gridK, blockK>>>(threads, (uint2*)d_hash);
