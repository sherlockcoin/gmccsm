...
extern "C" int scanhash_spreadx11( int thr_id, struct work *work, uint32_t max_nonce, unsigned long *hashes_done )
{
	...
	if (opt_throughput == 10)
	{
		...
		{
			throughput = (12) * 1024 * 64;
		}
	}

	uint32_t simdthreads = 192;
	// smid validiert ueber 256 nicht mehr. 256 ist langsamer als 128
	uint32_t shavitethreads = 256; 
	// shavite ...
	uint32_t luffacubehashthreads = 512;
	// smid ... 
	//device sm geht noch nicht
    ...
    if (!init[thr_id])
    {
		...
		//x11_shavite512_cpu_init(thr_id, throughput, shavitethreads);
...
//		gettimeofday(&tv_start, NULL);
		...
		quark_groestl512_cpu_hash_64(throughput, nonce, NULL, d_hash[thr_id]);
 //       PRINTTIME("groestl");
...
//		gettimeofday(&tv_start, NULL);
		...
		x11_luffaCubehash512_cpu_hash_64(throughput, nonce, d_hash[thr_id], luffacubehashthreads);
 //       PRINTTIME("luffaCubehash");

//		gettimeofday(&tv_start, NULL);
		//x11_shavite512_cpu_hash_64( throughput, nonce, d_hash[thr_id]);
		x11_shavite512_cpu_hash_64( throughput, nonce, d_hash[thr_id], shavitethreads);
//        PRINTTIME("shavite");

//		gettimeofday(&tv_start, NULL);
		x11_simd512_cpu_hash_64(thr_id,throughput, nonce, d_hash[thr_id], simdthreads);
 //       PRINTTIME("simd");
