...
__global__ void spreadx11_sha256double_gpu_hash_88( int threads, uint32_t startNonce, uint32_t *g_hash )
{
    int thread = (blockDim.x * blockIdx.x + threadIdx.x);
    if (thread < threads)
    {
...
*/
    ...
    cudaMemcpyAsync(d_wholeblock, paddedmessage, 200000, cudaMemcpyHostToDevice);
}
